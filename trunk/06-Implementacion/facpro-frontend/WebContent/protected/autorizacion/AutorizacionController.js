caApp.registerCtrl('autorizacionAdminController', function($scope, $location,
		$http, ngTableParams, breadcrumbs, $cookieStore, webServices,
		normalizeString, FiltroFactura, tableFactory, IndexFactura,
		utilidadesConversion, tableCollapseFactory, paginatedFunction, AModel, utilidades, FiltroFactura) {
	...
	$scope.goAuthorize = function($param, $str1, $index) {
		...
		var prompt = str + "Esta seguro de autorizar la factura?";
	...
	$scope.paginatedAutoName='';
	$scope.filterName='filter';
	paginatedFunction.generatePaginatedMethod($scope, 'paginatedAutoName', $scope.rollbackFilterFields, 'filter.page', $scope.loadFacturas,
