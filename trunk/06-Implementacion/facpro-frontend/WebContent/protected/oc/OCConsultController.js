caApp.registerCtrl('oCAdminConsultController', function($scope, $location,
	...
	$scope.openModalAssociateFacturas = function (ordenId, clienteId, ordenNumero, saldo, index) {		
		$scope.closeAllRowsOc(ordenId, index);
		$scope.ordenNumero = ordenNumero;
		$scope.saldo = utilidades.getCurrencyNumber(saldo);
		...
		var callbackFuntion = function(data, status, headers, config) {
			...
			$scope.tableAssociate = tableFactory.getTableFilterData(
					...
					$scope, 4);
			$scope.montoInicial = $scope.calcularTotalSeleccionado();
	...
	$scope.calcularTotalSeleccionado = function(){
		var total = 0;
		angular.forEach($scope.facturaPorOC.facturaCliente, function(value, key) {
	...
	$scope.associateFacturas = function () {
		...
		var total = $scope.calcularTotalSeleccionado();
		var montoNvaFac = Math.round((Number(total)-Number($scope.montoInicial))*100)/100;
		var saldoActualizado = Number($scope.saldo)-montoNvaFac;
		if(saldoActualizado<=0){
			var buttons = ['Extender OC', 'Cerrar OC'];
			alertify.custom("<b>El saldo de la OC es negativo.  </br>" +
					"Desea Extender o Cerrar la misma?" , buttons, function (e) {
				if(typeof e === 'string'){
					if (e == "Extender OC") {
						$scope.facturaPorOC.estado = utilidades.getEstadosOC().EXTENDIDA;
					} else if (e == "Cerrar OC") {
						$scope.facturaPorOC.estado = utilidades.getEstadosOC().CERRADA;
					}
					$scope.asociarFacturaWs(callBackFuntion);
				}
			});
		}else{
			$scope.asociarFacturaWs(callBackFuntion);
	...
	$scope.asociarFacturaWs = function (callBackFuntion){
