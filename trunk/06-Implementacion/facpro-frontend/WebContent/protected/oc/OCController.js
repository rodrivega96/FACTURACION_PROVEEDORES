caApp.registerCtrl('oCAdminController', function($scope, $location, $http,
		...
		FiltroOC, paginatedFunction, OCModel, utilidades) {
	...
	$scope.goVigente = function(id, numero, estadoId) {
		...
		var errorOc = "La Orden de Compra <b>" + numero + "</b> no se puede cambiar de estado porque esta desactualizada.";
		sendData = {
				'id' : id,
				'estadoId' : estadoId
		};
		$scope.updateOc('changeStateOrden', sendData, str, success, fail, errorOc, errorStatus);
	};
	
	
	$scope.goCerrar = function(id, numero, estadoId) {
		...
		$scope.updateOc('closeOrden', sendData, str, success, fail, errorOc);
	};
	...
	$scope.updateOc = function (ws, id, question, success, fail, errorOc, errorStatus){
		    ...
		    if (e) {
		    	var callbackFuntion = function(data, status, headers, config) {
		    		if(data.status==0){
		    			...
		    			alertify.inform(errorOc != null ? errorOc : fail);
		    			$scope.loadFilter();
		    		} else {
