...
public class LoginWebService {
	...
	public LoginResponseDTO login(LoginDTO loginDTO) {
		...
		response.setMessage("");
		response.setToken("");
		response.setVersion(ldapService.getVersion());
		...
		try {
			...
			response.setMenu(facProMenuLoader
					.generateMenuView(facProAuthenticator.getRolesUserMenu(user)));
			response.setToken(crd.getSessionId());
			response.setUserId(user.getId());
			response.setStatus(STATUS_OK);
	...
	public LoginResponseDTO getMenu(LoginDTO loginDTO) {
		...
		response.setMenu(facProMenuLoader.generateInitialMenu());
		try {
			UserView user = facProAuthenticator.getUser(loginDTO.getUserName());
			response.setMenu(facProMenuLoader
					.generateMenuView(facProAuthenticator.getRolesUserMenu(user)));
		} catch (AccessDeniedException e) {
			response.setStatus(STATUS_INVALID_USER);
