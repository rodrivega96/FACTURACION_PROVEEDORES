...
//	}
	...
	public BasicCredential authenticate(UserView user) {
		...
		// Stores in cache the user info associated with the session ID.
		cacheManager.getCache("defaultCache").put(credential.getSessionId(),
				UserDTO.of(user, userService.findUserRoles(user.getId())));
		return credential;

	}

	public UserView getUser(String login) {
		UserView user = userService.findView(login);
		if (user == null) {
			throw new AccessDeniedException(
					messageResource.getMessage("error.unauthorized.access"));
		}
	...
	public List<RoleView> getRolesUserMenu(UserView user) {
		return userService.getUserRolesMenu(user.getId());
	}

	public UserView getUser(String login, String password) {
		UserView user = userService.findView(login, password);
		if (user == null) {
			throw new AccessDeniedException(
					messageResource.getMessage("error.unauthorized.access"));
