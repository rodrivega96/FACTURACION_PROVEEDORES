caApp.registerCtrl('oCFormController', function($scope, $location, $http,
		...
		$filter, normalizeString, download, utilidades, utilidadesConversion, OCModel) {
...
caApp.registerCtrl('oCEditController', function($scope, $location, $http,
		ngTableParams, breadcrumbs, $cookieStore, webServices, tableFactory,
		$routeParams, normalizeString, download, utilidades, utilidadesConversion, OCModel) {
	...
	$scope.saveOC = function() {
		if ($scope.oc.clienteId == '') {
			...
			var callBackFuntion = function(data, status,
					headers, config) {
				if (data.status == 0) {
					alertify.success("La Orden de Compra numero <b>" + $scope.oc.numero
							+ "</b> se actualizo correctamente");
					window.history.back();
				} else if (data.status == 1) {
					alertify.confirm("La Orden de Compra numero <b>" + $scope.oc.numero
							+ "</b> ha sido modificada por otra persona, desea "
