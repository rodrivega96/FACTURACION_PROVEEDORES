...
import com.vates.facpro.business.states.Inicial;
...
public class FacturaWebService {
	...
	public Page<FacturaView> getPaginatedNivelInferiorList(
		...
		return facturaAdmService.findPaginatedPendingFilteredList(userId,
				pageIndex - 1, rowsPerPage, variable, order);
	}
	...
	private Factura createFatura(FacturaFullDTO dto) {
		...
		fac.setFacturaAdm(facturaAdmService.find(dto.getIdFacturaAdm()));
		return fac;
	...
	public FacturaResponseDTO saveFactura(FacturaFullDTO factura) {
		...
		try {
			...
			facturaService.saveFactura(facturaSave);
			response.setStatus(STATUS_OK);
