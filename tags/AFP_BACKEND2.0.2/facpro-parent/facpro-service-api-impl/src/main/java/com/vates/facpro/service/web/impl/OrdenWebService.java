...
public class OrdenWebService {
	...
	public ConsultarOrdenDTO getPaginatedConsultList(
		...
		dto.setOrdenView(ordenService.findPaginatedFilteredConsultList(clienteNombre, clienteId, fvDesde, fvHasta,
				proyectoId, managerId, numero, estadoOC, estadoPreFac, pageIndexOC - 1, rowsPerPageOC));
		dto.setPreFacView(ordenService.findPaginatedFilteredPreFacList(clienteNombre, clienteId, fvDesde, fvHasta,
				proyectoId, managerId, numero, estadoOC, estadoPreFac, pageIndexPF - 1, rowsPerPagePF));
		return dto;
	...
	@Consumes(MediaType.APPLICATION_JSON)
	public OrdenResponseDTO estadoOrden(Long id,
			@HeaderParam(value = "vates-session-id") String sessionId) {
		OrdenResponseDTO response = new OrdenResponseDTO();
		try {
			UserDTO userDTO = (UserDTO) this.cacheManager
			...
			Orden oc = ordenService.getOrden(id);
			oc.setUpdatedBy(userDTO.getId());
			oc = ordenService.updateOrdenChangeState(oc, userDTO.getId(), null);
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (OptimisticLockException e) {
			response.setMessage(e.getMessage());
	...
	@Path("/closeOrden")
	@Consumes(MediaType.APPLICATION_JSON)
	public OrdenResponseDTO closeOrden(Long id,
			@HeaderParam(value = "vates-session-id") String sessionId) {
		OrdenResponseDTO response = new OrdenResponseDTO();
		try {
			UserDTO userDTO = (UserDTO) this.cacheManager
	...
	public OrdenResponseDTO getFacturaClienteByOrdenId(
		...
		OrdenResponseDTO response = new OrdenResponseDTO();
		try {
			response.setFacturaList(ConversorDTO.getFacturasList(ordenService.findFacturasById(id)));
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (Exception ex) {
			response.setMessage(ex.getMessage());
