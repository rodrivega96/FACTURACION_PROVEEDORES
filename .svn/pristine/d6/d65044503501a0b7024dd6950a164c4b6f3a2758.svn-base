...
public class OrdenWebService {
	...
	public OrdenResponseDTO saveOrden(OrdenCompraDTO dto,
		...
		Orden oc = null;
		try {
			UserDTO userDTO = (UserDTO)ordenService.getUserDto(cacheManager, sessionId);
	...
	public OrdenResponseDTO getDatoInicial(
			final @QueryParam("load") Boolean load) {
		OrdenResponseDTO response = new OrdenResponseDTO();
		try {
			DatoInicialOrdenDTO datoDTO = getDatosIniciales(
					AdministradorEstadosOC.estadosIniciales, !load);
	...
	public OrdenResponseDTO getOrdenById(final @QueryParam("id") Long id,
		...
		try {
			...
			response.setDatoInicialDTO(getDatosIniciales(
					AdministradorEstadosOC.estados.get(orden.getEstado())
							.getProximosEstados(), !load));
			response.setDto(ConversorDTO.getOrdenCompraDTO(orden));
			response.getDto().setProyectos(cargarProyectos(orden));
			response.setArchivosList(ConversorDTO.getArchivoOrden(
					archivoOrdenService.findByOrdenIdAndDelete(id, false),
					orden.getEstado()));
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (Exception ex) {
	...
	public OrdenesResponseDTO getFiltroInicial() {
		...
		try {
			DatoInicialOrdenDTO datoDTO = getDatosIniciales(new HashSet<EstadoOC>(
					AdministradorEstadosOC.estados.values()), false);
	...
	private DatoInicialOrdenDTO getDatosInicialesGenericos() {
	...
	private DatoInicialOrdenDTO getDatosIniciales(Set<EstadoOC> estados, boolean onlySates) {
		DatoInicialOrdenDTO datoDTO  = null;
		if(onlySates){
			datoDTO = new DatoInicialOrdenDTO();
		}else{
			datoDTO = getDatosInicialesGenericos();
		}
		...
		try {
			...
			ordenService.saveAsociarFactura(
					ConversorDTO.getFacturaPorOC(dto.getFacturaCliente(),
							dto.getOrdenId(), userDTO.getId()), dto.getOrdenId(), dto.getEstado());
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (OptimisticLockException e) {
			response.setMessage(e.getMessage());
