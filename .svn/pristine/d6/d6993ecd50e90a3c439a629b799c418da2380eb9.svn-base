caApp.controller('fileAdminController', function($http, $scope, $location,
		...
		webServices, tableFactory, download, normalizeString, utilidades, utilidadesConversion) {
	...
	$scope.goWF = function() {
		if($scope.canGoWF==true){
			$location.path("/factura-admin/factura-wf-form-edit/" + $routeParams.facId);
		}else{
			alertify.error("Debe subir un archivo de tipo Factura para continuar");
		}
