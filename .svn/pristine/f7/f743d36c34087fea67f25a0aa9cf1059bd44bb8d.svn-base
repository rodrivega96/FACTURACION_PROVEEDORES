...
public class HistorialAutorizacionWebService {
	...
	public HistorialAutorizacionResponseDTO getHistorialByFinalUser(
			final @QueryParam("idFactura") Long idFactura, @QueryParam("simp") Boolean simp, @QueryParam("onlyDet") Boolean onlyDet) {
		HistorialAutorizacionResponseDTO response = new HistorialAutorizacionResponseDTO();
		try {
			response.setDto(onlyDet?getHistorialFacturaOnlyDetDTO(idFactura):getHistorialFacturaDTO(idFactura, simp));
	...
	@Path("/saveHistorial")
	@Consumes(MediaType.APPLICATION_JSON)
	public HistorialAutorizacionResponseDTO saveHistorial(AutorizacionDTO dto, @HeaderParam(value = "vates-session-id") String sessionId) {
		HistorialAutorizacionResponseDTO response = new HistorialAutorizacionResponseDTO();
		try {
			UserDTO userDTO = (UserDTO)historialService.getUserDto(cacheManager, sessionId);
			historialService.saveHistorial(dto.getId(), dto.getDescripcion(),
						dto.getEstado(), userDTO.getId());
			response.setStatus(STATUS_OK);
			response.setMessage("");
			...
			response.setMessage("La Factura no esta actualizada recargue la página y vuelva a intentarlo");
			response.setStatus(STATUS_DATABASE_EXCEPTION);
		} catch (Exception ex) {
			response.setMessage(ex.getMessage());
			ex.printStackTrace();
	...
	public HistorialAutorizacionResponseDTO rechazarFacturaHistorial(
			AutorizacionDTO dto,  @HeaderParam(value = "vates-session-id") String sessionId) {
		HistorialAutorizacionResponseDTO response = new HistorialAutorizacionResponseDTO();
		try {
			UserDTO userDTO = (UserDTO)historialService.getUserDto(cacheManager, sessionId);
			historialService.rechazarFacturaHistorial(dto.getId(),
						dto.getDescripcion(), userDTO.getId());
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (MessagingException ex) {
			response.setMessage(ex.getMessage());
	...
	private HistorialAutorizacionDTO getHistorialFacturaDTO(Long idFactura, Boolean simp) {
		Factura f = null;
		HistorialAutorizacionDTO dto = new HistorialAutorizacionDTO();
		if(!simp){
			FacturaAdm fa = facturaAdmService.find(idFactura);
			f = fa.getFactura();
			dto = getHistorialFacturaComunDTO(idFactura, fa, f);
		}else{
			dto = new HistorialAutorizacionDTO();
			f = facturaService.findByAdmId(idFactura);
		}
		...
		for (HistorialAutorizacion his : historialService.findByuserAndFactura(f.getId())) {
			...
			newDTO.setAutoriza(userService.getUserNameAndLastName(his.getNivel().getUsuarioId()));
		...
		if (f.getEstado().equals(Estados.EN_AUTORIZACION)) {
			for (Nivel n : nivelService.findNivelByFacturaId(f.getId())) {
				if (!n.getEliminado()
						&& f.getNivel().getOrden() <= n
								.getOrden()) {
					HistorialWFDTO newDTO = new HistorialWFDTO();
					newDTO.setNivel(n.getOrden());
					newDTO.setAutoriza(userService.getUserNameAndLastName(n.getUsuarioId()));
	...
	private HistorialAutorizacionDTO getHistorialFacturaOnlyDetDTO(Long idFactura){	
		FacturaAdm fa = facturaAdmService.find(idFactura);
		Factura f = fa.getFactura();
		return getHistorialFacturaComunDTO(idFactura, fa, f);
	}
	
	private HistorialAutorizacionDTO getHistorialFacturaComunDTO(Long idFactura, FacturaAdm fa, Factura f){
		HistorialAutorizacionDTO dto = new HistorialAutorizacionDTO();
		...
		List<FacturaAsientoAdm> listAsientos = facturaAdmService.getFilteredFacturaAsiento(fa.getAsientos());
		for (FacturaAsientoAdm asiento : listAsientos) {
			if(!exluidas.contains(asiento.getJerarquia())){
				...
				as.setImporteNeto(ParseoService.alwaysTwoDecimal(asiento
						.getImporteNeto()));
	...
	public HistorialAutorizacionResponseDTO informarFacturaHistorial(
			final @QueryParam("id") Long idFactura, final @QueryParam("descripcion") String descripcion,
			@HeaderParam(value = "vates-session-id") String sessionId) {
		HistorialAutorizacionResponseDTO response = new HistorialAutorizacionResponseDTO();
		try {
			UserDTO userDTO = (UserDTO)historialService.getUserDto(cacheManager, sessionId);
			historialService.informar(idFactura, userDTO.getId(), descripcion);
			response.setStatus(STATUS_OK);
		} catch (Exception ex) {
			response.setMessage(ex.getMessage());
			response.setStatus(STATUS_DATABASE_EXCEPTION);
