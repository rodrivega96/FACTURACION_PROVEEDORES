...
public class FacturaWebService {
	...
	private static final int STATUS_EXIST_KANAV = 3;
	...
	public Page<FacturaView> getPaginatedList(
		...
		return facturaAdmService.findPaginatedAdminFilteredList(numero, tipo, cuit,
	...
	public Page<FacturaView> getPaginatedList(
		...
		UserDTO userDTO = (UserDTO)facturaAdmService.getUserDto(cacheManager, sessionId);
		return facturaAdmService.findPaginatedFilteredListAuth(numero, tipo, cuit,
				razon, cc, centroc, fvDesde, fiDesde, flfDesde, fvHasta,
				fiHasta, flfHasta, ffacDesde,ffacHasta ,userDTO.getId(),
	...
	public Page<FacturaView> getPaginatedNivelInferiorList(
			...
			final @QueryParam("ffacDesde") String ffacDesde,
			final @QueryParam("ffacHasta") String ffacHasta,
			final @QueryParam("userSelected") String userSelected) {
		UserDTO userDTO = (UserDTO)facturaAdmService.getUserDto(cacheManager, sessionId);
		return facturaAdmService.findPaginatedPendingFilteredListLowLevel(numero, tipo, cuit,
				razon, cc, centroc, fvDesde, fiDesde, flfDesde, fvHasta,
				fiHasta, flfHasta, ffacDesde,ffacHasta ,userDTO.getId(),
				pageIndex - 1, rowsPerPage, variable, variableLocal, order, userSelected);
	}
	...
	private FacturaFullDTO getFacturaFullDTO(Long id) {
		...
		dto.setImporteNeto(ParseoService.alwaysTwoDecimal(fa.getGralBruto()));
		dto.setTotal(ParseoService.alwaysTwoDecimal(fa.getGralNeto()));
		dto.setIva(ParseoService.alwaysTwoDecimal(fa.getGralIvaInc()));
	...
	public FacturaResponseDTO saveFactura(FacturaFullDTO factura) {
		...
		try {
			...
			Long facturaAdmId = facturaService.findFacturaAdmByNroKanav(factura
					.getNroKanav(), factura.getIdFacturaAdm());
			if (factura.getNroKanav() == null || facturaAdmId == null) {
				...
				response.setFacturaNro(facturaService.findFacturaNroByFacturaAdmId(facturaAdmId));
				response.setStatus(STATUS_EXIST_KANAV);
			}
	...
	public Page<FacturaView> getPaginatedList(
		...
		return facturaAdmService.findPaginatedFilteredList(numero, tipo, cuit,
				razon, cc, centroc, wf, fvDesde, fiDesde, flfDesde, fvHasta,
				fiHasta, flfHasta, ffacDesde, ffacHasta, userSelected, estado, pageIndex - 1,
	...
	@Path("/getPendingOk")
	@Consumes(MediaType.APPLICATION_JSON)
	public FacturaResponseDTO getPendingOk(
			@HeaderParam(value = "vates-session-id") String sessionId,
			final @QueryParam("id") Long id) {
		FacturaResponseDTO response = new FacturaResponseDTO();
		try {
			UserDTO userDTO = (UserDTO)facturaAdmService.getUserDto(cacheManager, sessionId);
			response.setListNivelView(facturaAdmService.getPendingOk(id,
					userDTO.getId()));
			response.setStatus(STATUS_OK);
			response.setMessage("");
		} catch (Exception ex) {
