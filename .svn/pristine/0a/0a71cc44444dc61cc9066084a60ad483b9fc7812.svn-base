caApp.registerCtrl('oCFormController', function($scope, $location, $http,
			...
			$cookieStore, function(data, status, headers, config) {
				if (data.status == 0) {
					...
					if($scope.loadAllInitialData){
						OCModel.loadInitialDataFilter($scope, data.datoInicialDTO);
						FiltroOC.initial = data.datoInicialDTO;
					}else{
						OCModel.loadInitialDataFilter($scope,FiltroOC.initial);
					}
	...
	$scope.goNewProject= function() {
		if($scope.oc.clienteId != ""){
			...
			alertify.error("Se debe seleccionar un cliente.");
			$('#clienteId').focus();		
		}
	...
	$scope.addProject = function() {
		if($scope.projects !== undefined){
			OCModel.addNewProject(false, OCModel, $scope.projects, $scope.oc);
			for (var i = 0; i < $scope.projects.length; i++) {
				OCModel.removeOldProject(i, $scope.projects, $scope.oc);				
			}
	...
	$scope.addFile = function() {
		...
		} else if ($scope.fileOc
				&& $scope.fileCont.typeFile == 2) {
		...
		var callBackFunction = function(data, status, headers, config) {
			if (data.status == 0) {
				if (utilidades.isPDF($file.name)) {
					...
					download.downloadFile(data.dto.content,
							$file.name, $file.type);
				}
			}
		};
...
caApp.registerCtrl('oCEditController', function($scope, $location, $http,
	...
	$scope.validateOrdenData = function () {
		$scope.showClientes = true;
	...
	$scope.addProject = function() {
		OCModel.addNewProject($scope.estadoOc == 1 ? false : true, OCModel, $scope.projects, $scope.oc);
		for (var i = 0; i < $scope.projects.length; i++) {
			OCModel.removeOldProject(i, $scope.projects, $scope.oc);
		}
		if ($scope.oc.proyectos.length < 1) {
	...
	$scope.forceUpdate = function (ordenNumero) {
		var callBackFuntion = function(data, status,
				headers, config) {
			if (data.status == 0) {
				alertify.success(ordenNumero
						+ "se actualizo correctamente");
				$location.path(utilidades.getPath($cookieStore, true));
			} else if (data.status == 3) {
				alertify.error(ordenNumero
						+ "compra ya existe");
			} else {
				alertify.error("Error generando " + ordenNumero
						+ ": " + data.message);
			}
		};
...
caApp.registerCtrl('oCViewController', function($scope, $location, $http,
		ngTableParams, breadcrumbs, $cookieStore, webServices, tableFactory,
		$routeParams, download, utilidades, utilidadesConversion, OCModel,
		FiltroOC) {
