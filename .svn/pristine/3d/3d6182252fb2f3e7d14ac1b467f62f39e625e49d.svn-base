										...
										function(data, status, headers, config) {
											if (data.status == 0) {
												...
												if($scope.wizardMode==true){
													$location.path("/factura-admin/file-form/" + data.factura.id);
												}else{
...
caApp.controller('facturaConsultAdminController', function($scope, $location, $http,
	...
	$scope.loadFacturas = function() {
		...
		webServices.getWSResponseGet($http, 'factura', 'facturaConsultPaginatedList',
				$scope.filter, $cookieStore, callbackFuntion);
	};
...
caApp.controller('fileConsultController', function($scope, $location,
		...
		$routeParams, tableFactory, $filter, download, utilidades, utilidadesConversion) {
...
caApp.controller('consultWFController', function($scope, $location, $http,
	...
	$scope.imprimir = function() {
		...
		$scope.scroll = 0;
		$scope.pdfUrl = printFactory.printVatesReport($scope.dataView, 'p', 'letter', 'protected/template/templatePDF.html', 'uint8array');
		$scope.showPDF=true;
